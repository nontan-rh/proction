...
async function runPlan(
  ...
  try {
    ...
    while (true) {
      ...
      scheduler.spawn(invocation.body!)
        ...
        .catch((err: unknown) => {
          ...
          erroredInvocations.add(invocation.id);
        })
        .finally(() => {
    ...
    if (invocationErrors.length > 0) {
      // Failed invocations may have partially written their destinations;
      // drop their committed records so the next submission re-executes them
      // instead of trusting the record. Records produced by this run are
      // never committed on failure.
      if (pruneResult != null) {
        for (const id of erroredInvocations) {
 ...
 * of a successful run or to drop the records of failed invocations.
 */
type PruneResult = {
  graphRun: GraphRun;
 ...
 * before prepareInvocations and prepareDataSlots so that blocker counts,
 * consumer counts, and reference counts are calculated only on the surviving
 * invocations.
...
function pruneUpToDateInvocations(plan: Plan): PruneResult {
  ...
  const { consumersByHandle, blockerCounts } = buildDependencyMaps(
    invocations,
  );
  ...
  for (const invocation of invocations.values()) {
    if (blockerCounts.get(invocation.id) === 0) {
  ...
  for (let i = 0; i < order.length; i++) {
    for (const output of order[i].outputs) {
      ...
      for (const consumer of consumers) {
        const count = blockerCounts.get(consumer.id)! - 1;
        blockerCounts.set(consumer.id, count);
        if (count === 0) {
          order.push(consumer);
        }
      }
