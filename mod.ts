 ...
 * - Incremental calculation across runs driven by data versions
 *
 * Each feature is provided in a modular, customizable way, and you can combine
 * them as you like.
 *
 * Incremental calculation: when versions are supplied to `$s` and `$d`, runs
 * on the same {@link Context} skip invocations whose inputs are unchanged and
 * whose destinations already hold the recorded content. Source versions are
 * non-negative integers managed by the caller and must change whenever the
 * content changes; destination versions are generated by the library in a
 * namespace of its own, reported through the `setVersion` callback of `$d`
 ...
 * means changed content. They are reported via {@link SetVersionFn} and must
 * be passed back to `$d` verbatim. Versions of sources are plain non-negative
 * integers managed by the caller instead.
 */
export type Version = DataVersion;

/**
 * A callback to receive the version of the content written to a destination.
 * It is called after a run finishes successfully. An exception thrown by the
 * callback is routed to the context's `reportError` and does not fail the
 * run.
 */
...
/**
 * A type to represent a middleware function. Middlewares wrap the execution
 * of an invocation; an invocation skipped as up-to-date by an incremental
 * run does not execute, so its middlewares are not invoked.
...
type IntermediateSlot = {
  ...
  // The stable identity of the provider across runs (the caller's provide
  // function, not the per-wiring wrapper); part of the invocation signature
  // because a different provider can change the produced content.
  provideKey: object;
 ...
 * invocations. It is the single source of truth for the edge semantics (one
 * edge per input reference) shared by the incremental pruning pass and the
 * execution preparation, so the two passes can never disagree on ordering.
