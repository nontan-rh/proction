...
When the similar `run` is executed repeatedly on one long-lived application, much of the work is often redundant. If some inputs have not changed since the last run, the destinations already hold the correct results. Proction can skip such calculations when you tell it how to recognize unchanged data, using "versions".

A version is an opaque non-negative integer describing the content of an external resource. Versions are compared only for equality. A different version means different content but its order does not mean anything.
...
```ts
import { Context, run, type Version } from "jsr:@nontan-rh/proction";
