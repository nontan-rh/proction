...
When the same `run` body is submitted repeatedly on one long-lived `Context`, much of the work is often redundant: if the inputs have not changed since the last run, the destinations already hold the correct results. Proction can skip such calculations when you tell it how to recognize unchanged data, using versions.

A version is an opaque non-negative integer describing the content of an external resource. Versions are compared only for equality: a different version means different content.

- For sources, you manage the version yourself and pass it as the second argument of `$s`. Change it (e.g. increment it) whenever you change the content of the object.
- For destinations, versions are generated by Proction. Pass a `setVersion` callback as the third argument of `$d` to receive the version of the written content, and pass that stored version back as the second argument on the next run.
...
const output = [0, 0, 0];
let outputVersion: Version | undefined = undefined;

async function calculate() {
  await run(ctx, ({ $s, $d }) => {
    const s = pureDouble($s(input, inputVersion));
    addProc($d(output, outputVersion, (v) => (outputVersion = v)), s, $s(input, inputVersion));
  });
}

await calculate(); // Calculates and writes `output`
await calculate(); // Nothing changed: all procedures are skipped

input[0] = 10;
inputVersion++;   // Tell Proction the content changed
await calculate(); // Recalculates
```

A calculation is skipped only when its inputs are unchanged and its results are confirmed to be present: either the destination version you passed matches the recorded one, or the intermediate result is only needed by calculations that are themselves skipped. When only part of the input changes, only the affected part of the dataflow is re-executed.
...
Incremental runs rely on two rules:

- Procedure bodies must be deterministic and must not have side effects other than writing their declared outputs.
- Versions must be kept honest: bump a source version whenever you mutate the source, and report a destination version only from `setVersion`. `setVersion` is called after a run finishes successfully; a failed run reports no versions. An exception thrown by `setVersion` is routed to the context's `reportError` and does not fail the run.
